...
@Component({
  selector: 'app-report-dashview',
  templateUrl: './report-dashview.component.html',
  styleUrls: ['./report-dashview.component.scss']
})
export class ReportDashviewComponent implements OnInit {
  reportdashview: any[] = [];
  filteredData: any[] = [];
  searchTerm: string = '';
  val:string| null='';
  ...
  fetchInfDetails(): void {
    this.http.get<any[]>(`${environment.serverUrl}api/report-dashpdf?encodedValue=${this.val}`)
      .subscribe(
        data => {
          console.log('Fetched data:', data);
          this.reportdashview = data;
          this.filteredData = data; // Initialize filteredData with fetched data
          console.log('dash', this.reportdashview);
          setTimeout(() => {
  ...
  viewPdf(Id: string): void {
    const url = `${environment.serverUrl}api/reportdashdown/${Id}`;
  ...
  onSearch(event: Event): void {
    ...
    this.filteredData = this.reportdashview.filter(item =>
