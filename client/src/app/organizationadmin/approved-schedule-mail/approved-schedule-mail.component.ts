...
@Component({
  selector: 'app-approved-schedule-mail',
  templateUrl: './approved-schedule-mail.component.html',
  styleUrls: ['./approved-schedule-mail.component.scss']
})
export class ApprovedScheduleMailComponent {
  ...
  getUsers(): void {
    this.http.get(`${environment.serverUrl}api/users_cc`).subscribe(
      (response: any) => {
        this.users = response;
      },
      (error) => {
  ...
  addUser(): void {
    if (this.email) {
      ...
      this.http.post(`${environment.serverUrl}api/add_mail_cc`, user).subscribe(
        response => {
  ...
  updateUser(): void {
    if (this.email && this.editingUserEmail) {
      ...
      this.http.put(`${environment.serverUrl}api/update_mail_cc`, user).subscribe(
        response => {
  ...
  deleteUser(userId: number): void {
    this.http.delete(`${environment.serverUrl}api/delete_mail_cc/${userId}`).subscribe(
      response => {
